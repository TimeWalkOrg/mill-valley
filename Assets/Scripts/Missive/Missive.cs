...
public class Missive
{
	...
	/// For internal message, not network messages
	/// </summary>
	...
	public static void AddListener<T>(string messageName, Action<T> callback) where T : Missive
	{
	...
	public static void RemoveListener<T>(string messageName, Action<T> callback) where T : Missive
	{
	...
	{
		SendMissive<Missive>(TypelessMessagePrefix + messageName, null);
	}
	...
	public static void Send<T>(T message) where T : Missive
	{
		SendMissive<T>(typeof(T).ToString(), message);
	}
	...
	/// <param name="messageName">The name of the message.</param>
	/// <param name="message">The instance of the message.</param>
	public static void Send<T>(string messageName, T message) where T : Missive
	{
		SendMissive<T>(typeof(T).ToString() + messageName, message);
	}
	...
	private static void RegisterListener(string messageName, Delegate callback)
	{
		if (callback == null)
		{
			UnityEngine.Debug.LogError("Failed to add SofaMessage Listener because the given callback is null!");
			return;
		}
	...
	private static void SendMissive<T>(string messageName, T e) where T : Missive
	{
		if (!handlers.ContainsKey(messageName)) { return; }
		...
		{
			if (messageHandlers[i].GetType() != typeof(Action<T>) && messageHandlers[i].GetType() != typeof(Action)) { continue; }

			if (OnMessageHandle != null) //Application.isEditor
			{
				string methodName = messageHandlers[i].Method.Name;
				...
				OnMessageHandle(callerType, messageHandlers[i].Target.GetType(), typeof(T), messageName, methodName);
			}

			if (typeof(T) == typeof(Missive))
			{
				Action action = (Action)messageHandlers[i];
				action();
			}
			else
			{
				Action<T> action = (Action<T>)messageHandlers[i];
				action(e);
			}
		}
	}
